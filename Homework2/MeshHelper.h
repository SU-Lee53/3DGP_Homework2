...
class MeshHelper {
public:
	static void CreateCubeMesh(ComPtr<ID3D12Device> pd3dDevice, ComPtr<ID3D12GraphicsCommandList> pd3dCommandList, std::shared_ptr<Mesh<DiffusedVertex>> pMesh,
		float fWidth = 4.0f, float fHeight = 4.0f, float fDepth = 4.0f, const XMFLOAT4& xmf4Color = XMFLOAT4{0.f, 0.f, 0.f, 1.f});

	static void CreateWallMesh(ComPtr<ID3D12Device> pd3dDevice, ComPtr<ID3D12GraphicsCommandList> pd3dCommandList, std::shared_ptr<Mesh<DiffusedVertex>> pMesh,
	...
	static BOOL CreateMeshFromOBJFiles(ComPtr<ID3D12Device> pd3dDevice, ComPtr<ID3D12GraphicsCommandList> pd3dCommandList,
		std::shared_ptr<Mesh<DiffusedVertex>> pMesh, std::wstring_view wstrObjPath, const XMFLOAT4& xmf4Color = XMFLOAT4{ 0.f, 0.f, 0.f, 1.f });

	static void CreateRollercoasterRailMesh(ComPtr<ID3D12Device> pd3dDevice, ComPtr<ID3D12GraphicsCommandList> pd3dCommandList,
